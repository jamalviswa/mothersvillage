...
public function update_stprofile(Request $request,$id) {
    ...
    public function institute_categories(Request $request) {
        ...
        try {
            $params = json_decode($request->getContent());
            $data = array();
            $cats = Institutecategory::where('status', 'Active')->get();
            if (!empty($cats)) {
                foreach ($cats as $cat) {
                    $data[] = array(
                        'id' => $cat->id,
    ...
    public function institutes(Request $request, $id) {
        ...
        try {
            $params = json_decode($request->getContent());
            $data = array();
            $institutes = Institute::where('status', 'Active')->where('category', $id)->get();
            if (!empty($institutes)) {
        ...
        try {
            ...
            if (!empty($institute)) {
               ...
               $data['details'][] = array(
                    ...
                    'payment_status' => !empty($find_payment) ? $find_payment->payment_status : "Pending",
            ...
            //         $cid= $countries->id;
            //         $docs = Document_list::where('status', 'Active')->where('institute_id', $id)->where('country_id', $cid)->get();
            //       //print_r($data);
            //       if(!empty($docs)){
            //         foreach($docs as $doc){
            //          $data['required_documents'][]=array(
            //          'country' => $countries->country,
            //           'document_list' => $doc->documents
            //          );
...
public function create_application(Request $request) {
    ...
    try {
        ...
        if (!empty($user_id)) {
            ...
            $data->studentSignature = $this->contentFormat($params->studentSignature);
            $data->studentDate = $this->contentFormat($params->studentDate);
            $data->parentName = $this->contentFormat($params->parentName);
            $data->parentSignature = $this->contentFormat($params->parentSignature);
            ...
            if(!empty($params->educational)){
                ...
                foreach($params->educational as $educational){ //ApplicationEducation
                    ...
                    $data1->nameOfSchools = $this->contentFormat(isset($educational->nameOfSchools)?$educational->nameOfSchools:'');
                    $data1->country = $this->contentFormat($educational->country);
                    $data1->state = $this->contentFormat(isset($educational->state)?$educational->state:'');
                    $data1->languageOfInstruction = $this->contentFormat(isset($educational->languageOfInstruction)?$educational->languageOfInstruction:"");
                    ...
                    $data1->certificateNo = $this->contentFormat(isset($educational->certificateNo)?$educational->certificateNo:"");
                    $data1->save();   
                }
            }
            }
            ...
            if(count($params->employment) > 0){
                foreach($params->employment as $employment){ //ApplicationEducation
                    ...
                    $data2->positionHeld = $this->contentFormat(isset($employment->positionHeld)?$employment->positionHeld:"");
                    $data2->natureOfDuties = $this->contentFormat(isset($employment->natureOfDuties)?$employment->natureOfDuties:"");
                    $data2->save();   
                }
            }
        }
            ...
            if(count($params->relations) > 0){
                foreach($params->relations as $relations){ //ApplicationEducation
                    ...
                    $data3->relationship = $this->contentFormat(isset($relations->relationship)?$relations->fullName:"");
                    $data3->dob = $this->contentFormat(isset($relations->dob)?$relations->dob:"");
                    $data3->placeOfBirth = $this->contentFormat(isset($relations->placeOfBirth)?$relations->placeOfBirth:"");
                    $data3->nationality = $this->contentFormat(isset($relations->nationality)?$relations->placeOfBirth:"");
                    $data3->nricNo = $this->contentFormat(isset($relations->nricNo)?$relations->nricNo:"");
                    $data3->finNo = $this->contentFormat(isset($relations->finNo)?$relations->finNo:"");
                    $data3->residentialStatusNone = $this->contentFormat(isset($relations->residentialStatusNone)?$relations->residentialStatusNone:"");
                    $data3->occupation = $this->contentFormat(isset($relations->occupation)?$relations->occupation:"");
                    $data3->parent_sibling = $this->contentFormat(isset($relations->parent_sibling)?$relations->parent_sibling:""); 
                    $data3->save();   
                }
            }
        }
...
public function health_send_request(Request $request){
        ...
        try{
            ...
            //  print_r($params->shedule_id);exit;
             ...
             $data->save();
             $data = new Notification();
             $data->to_id = $shedule->sprovider_id;
             $data->from_id = $params->user_id;
             $data->label = "Sending request to booking the clinic Appointment ";
             $data->save();
            $response = array("data" => $data,"message" =>"Your booking request sent", "code" => "200"); 
            return response()->json($response);
        }else{
...
public function health_req_list(Request $request){
        ...
        //   print_r($service->sprovider_id);
                  $data[]=array(
                    ...
                    "created_date"=>date('d-m-Y',$books->created_date),
                    "from_time"=>$shedule->from_time,
                    "to_time"=>$shedule->to_time,
         ...
         try {
           ...
           $user_amount=$user->wallet_amount;
            ...
            if($amount<=$user_amount){
                ...
                // print_r($total);exit;
                $provider=HealthBooking::where('id',$params->booking_id)->first();
             ...
             $notification = Notification::find($data->id);
              $user=User::where('user_id', $notification->to_id)->first();
               $msg = array('message' => $notification->label, 'node_id' => '1', 'action' => 'match');
                ...
                $this->send_push_notification($user->fcmid, $msg);
             $data = new Notification();
             $data->to_id = $provider->provider_id;
             $data->from_id = $user->user_id;
             $data->label = "Payment transfered!";
             $data->save();
             $notification = Notification::find($data->id);
